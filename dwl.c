...
#define BORDERPX(C)                                                           \
    (borderpx                                                                 \
     + ((C)->swallowing ? (int)ceilf (swallowborder * (C)->swallowing->bw)    \
                        : 0))
...
static void destroypointerconstraint (struct wl_listener *listener,
                                      void *data);
...
static struct wl_listener request_set_cursor_shape
    = { .notify = setcursorshape };
...
applyrules (Client *c)
{
    ...
    /*This line did not exist in the swallow patch*/
...
bool
bufdatabegin (struct wlr_buffer *wlr_buffer, uint32_t flags, void **data,
    ...
    for (i = 0; i < LENGTH (m->pool); i++)
        {
            ...
            buf = ecalloc (1,
                           sizeof (Buffer) + (m->b.width * 4 * m->b.height));
            buf->image
                = drwl_image_create (NULL, m->b.width, m->b.height, buf->data);
            wlr_buffer_init (&buf->base, &buffer_impl, m->b.width,
                             m->b.height);
            m->pool[i] = buf;
            break;
        }
    ...
    switch (event->state)
        ...
        case WL_POINTER_BUTTON_STATE_PRESSED:
            ...
            if (!c && !exclusive_focus
                && (node
                    = wlr_scene_node_at (&layers[LyrBottom]->node, cursor->x,
                                         cursor->y, NULL, NULL))
                ...
                && buffer == selmon->scene_buffer)
                {
                    ...
                    if (i < LENGTH (tags))
                        ...
                        click = ClkLtSymbol;
                                      ...
                                      - (TEXTW (selmon, stext) - selmon->lrpad
                                         + 2))
                        {
                            click = ClkStatus;
                        }
            ...
            for (b = buttons; b < END (buttons); b++)
                {
                    if (CLEANMASK (mods) == CLEANMASK (b->mod)
                        ...
                        && b->func)
                        {
                            b->func (click == ClkTagBar && b->arg.i == 0
                                         ? &arg
                                         : &b->arg);
                            return;
                        }
    ...
    wl_list_for_each (inhibitor, &idle_inhibit_mgr->inhibitors, link)
    {
        ...
        if (exclude != surface
            && (bypass_surface_visibility
                || (!tree
                    || wlr_scene_node_coords (&tree->node, &unused_lx,
                                              &unused_ly))))
            {
                inhibited = 1;
                break;
...
commitlayersurfacenotify (struct wl_listener *listener, void *data)
{
    ...
    if (l->layer_surface->initial_commit)
        {
            client_set_scale (layer_surface->surface,
...
commitnotify (struct wl_listener *listener, void *data)
{
    ...
    if (c->surface.xdg->initial_commit)
        {
             ...
             * Note that if the user set a rule in which the client is placed
             * on a different monitor based on its title, this will likely
             * select a wrong monitor.
             */
            applyrules (c);
            if (c->mon)
                {
                    client_set_scale (client_surface (c),
                                      c->mon->wlr_output->scale);
                }
            setmon (c, NULL, 0);
            /* Make sure to reapply rules in mapnotify() */
    ...
    if (client_surface (c)->mapped && c->mon
        && c->mon->lt[c->mon->sellt]->arrange && !c->isfullscreen
        && !c->isfloating)
            ...
            if (!r->name || strstr (wlr_output->name, r->name))
                {
                    ...
                    m->nmaster = r->nmaster;
                    m->lt[0] = r->lt;
                    m->lt[1] = &layouts[LENGTH (layouts) > 1
                                        && r->lt != &layouts[1]];
...
dirtomon (enum wlr_direction dir)
{
    ...
    if (!wlr_output_layout_get (output_layout, selmon->wlr_output))
        return selmon;
    if ((next = wlr_output_layout_adjacent_output (output_layout, dir,
                                                   selmon->wlr_output,
...
drawbar (Monitor *m)
{
    ...
    if (m == selmon)
        { /* status is only drawn on selected monitor */
            ...
            drwl_text (m->drw, m->b.width - tw, 0, tw, m->b.height, 0, stext,
                       0);
        }
    ...
    for (i = 0; i < LENGTH (tags); i++)
        {
            w = TEXTW (m, tags[i]);
            drwl_setscheme (
                m->drw, colors[m->tagset[m->seltags] & 1 << i ? SchemeSel
                                                              : SchemeNorm]);
    ...
    if ((w = m->b.width - tw - x) > m->b.height)
        {
            if (c)
                {
                    // ak: can be commented out to have black background on bar
                    // text
                    // drwl_setscheme ( m->drw, colors[m == selmon ? SchemeSel
                    // : SchemeNorm]);
    ...
    if (old && (!c || client_surface (c) != old))
        {
             ...
             * its color and focus it after the overlay is closed. */
            if (old_client_type == LayerShell
                ...
                {
                    return;
                    /* Don't deactivate old client if the new one wants focus,
...
focusstack (const Arg *arg)
{
    ...
    if (!sel || (sel->isfullscreen && !client_has_children (sel)))
        // if sel is null, or if sel is fullscreen and it has no children,
        // return
        return;
    if (arg->i > 0)
        {
    ...
    if ((p = client_get_parent (c)))
        {
            ...
            if (p->mon)
                {
                    c->geom.x
                        = (p->mon->w.width - c->geom.width) / 2 + p->mon->m.x;
                    c->geom.y = (p->mon->w.height - c->geom.height) / 2
                                + p->mon->m.y;
                }
...
motionabsolute (struct wl_listener *listener, void *data)
{
     ...
     * move the mouse over the window. You could enter the window from any
     * edge, so we have to warp the mouse there. Also, some hardware emits
     * these events.
     */
    ...
    if (time)
        {
            ...
            wl_list_for_each (constraint, &pointer_constraints->constraints,
                              link) cursorconstrain (constraint);

            if (active_constraint && cursor_mode != CurResize
                && cursor_mode != CurMove)
                {
                    toplevel_from_wlr_surface (active_constraint->surface, &c,
                                               NULL);
                    if (c
                        && active_constraint->surface
...
motionrelative (struct wl_listener *listener, void *data)
{
     ...
     * generated the event. You can pass NULL for the device if you want to
...
powermgrsetmode (struct wl_listener *listener, void *data)
{
    ...
    m->gamma_lut_changed
        = 1; /* Reapply gamma LUT when re-enabling the ouput */
...
run (char *startup_cmd)
{
    ...
    /* At this point the outputs are initialized, choose initial selmon based
     * on cursor position, and set default cursor image */
    selmon = xytomon (cursor->x, cursor->y);
...
setfullscreen (Client *c, int fullscreen)
{
    ...
    if (fullscreen)
        ...
        {
            /* restore previous size instead of arrange for floating windows
             * since client positions are set by the user and cannot be
             * recalculated */
            resize (c, c->prev, 0, 1);
        }
    arrange (c->mon);
...
setup (void)
{
     ...
     * and output hardware. The autocreate option will choose the most suitable
     ...
     * but we need to create the linux_dmabuf interface manually to integrate
     * it with wlr_scene. */
    ...
    /* Set up our client lists, the xdg-shell and the layer-shell. The
     * xdg-shell is a Wayland protocol which is used for application windows.
     * For more detail on shells, refer to the article:
     *
...
urgent (struct wl_listener *listener, void *data)
{
    ...
    if (client_surface (c)->mapped)
        client_set_border_color (
            c, (float[])COLOR (colors[SchemeUrg][ColBorder]));
}
    ...
    if (!c && selmon)
        {
            wlr_cursor_warp_closest (cursor, NULL,
             ...
             && (cursor->x < c->geom.x || cursor->x > c->geom.x + c->geom.width
                 || cursor->y < c->geom.y
          ...
          LayerSurface **pl, double *nx, double *ny)
{
    ...
    for (layer = NUM_LAYERS - 1; !surface && layer >= 0; layer--)
        {
            if (!(node
                  = wlr_scene_node_at (&layers[layer]->node, x, y, nx, ny)))
                continue;
...
configurex11 (struct wl_listener *listener, void *data)
{
    ...
    if (!client_surface (c) || !client_surface (c)->mapped)
        {
            wlr_xwayland_surface_configure (c->surface.xwayland, event->x,
                                            event->y, event->width,
                                            event->height);
            return;
        }
