...
void applybounds(Client *c, struct wlr_box *bbox) {
    ...
    if (c->geom.x >= bbox->x + bbox->width) c->geom.x = bbox->x + bbox->width - c->geom.width;
    if (c->geom.y >= bbox->y + bbox->height) c->geom.y = bbox->y + bbox->height - c->geom.height;
    if (c->geom.x + c->geom.width <= bbox->x) c->geom.x = bbox->x;
    if (c->geom.y + c->geom.height <= bbox->y) c->geom.y = bbox->y;
}
...
void arrange(Monitor *m) {
    ...
    if (!m->wlr_output->enabled) return;
    ...
    wl_list_for_each(c, &clients, link) {
        if (c->mon != m || c->scene->node.parent == layers[LyrFS]) continue;
...
void arrangelayers(Monitor *m) {
    ...
    for (i = 0; i < (int)LENGTH(layers_above_shell); i++) {
        wl_list_for_each_reverse(l, &m->layers[layers_above_shell[i]], link) {
            if (locked || !l->layer_surface->current.keyboard_interactive || !l->mapped) continue;
...
void bufdestroy(struct wlr_buffer *wlr_buffer) {
    ...
    if (buf->busy) wl_list_remove(&buf->release.link);
...
void buttonpress(struct wl_listener *listener, void *data) {
    ...
    case WL_POINTER_BUTTON_STATE_PRESSED:
        ...
        xytonode(cursor->x, cursor->y, NULL, &c, NULL, NULL, NULL);
        if (click == ClkClient && (!client_is_unmanaged(c) || client_wants_focus(c))) focusclient(c, 1);
...
void cleanupmon(struct wl_listener *listener, void *data) {
    ...
    if (m->lock_surface) destroylocksurface(&m->destroy_lock_surface, NULL);
...
void closemon(Monitor *m) {
    ...
    if (!nmons) {
        ...
        if (!selmon->wlr_output->enabled) selmon = NULL;
    }
    ...
    wl_list_for_each(c, &clients, link) {
        if (c->isfloating && c->geom.x > m->m.width)
            resize(c,
                   (struct wlr_box){
                       .x = c->geom.x - m->w.width, .y = c->geom.y, .width = c->geom.width, .height = c->geom.height},
                   0, 1);
        if (c->mon == m) setmon(c, selmon, c->tags);
    }
...
void commitnotify(struct wl_listener *listener, void *data) {
    ...
    if (c->surface.xdg->initial_commit) {
         ...
         */
        applyrules(c);
        if (c->mon) { client_set_scale(client_surface(c), c->mon->wlr_output->scale); }
    ...
    if (client_surface(c)->mapped && c->mon && c->mon->lt[c->mon->sellt]->arrange && !c->isfullscreen && !c->isfloating)
...
KeyboardGroup *createkeyboardgroup(void) {
    ...
    if (numlock) {
        ...
        if (mod_index != XKB_MOD_INVALID) locked_mods |= (uint32_t)1 << mod_index;
    }
    ...
    if (locked_mods) wlr_keyboard_notify_modifiers(&group->wlr_group->keyboard, 0, 0, locked_mods, 0);
...
void createlocksurface(struct wl_listener *listener, void *data) {
    ...
    if (m == selmon) client_notify_enter(lock_surface->surface, wlr_seat_get_keyboard(seat));
}
...
void createmon(struct wl_listener *listener, void *data) {
    ...
    if (!(m->drw = drwl_create())) die("failed to create drwl context");
...
void destroylocksurface(struct wl_listener *listener, void *data) {
    ...
    if (lock_surface->surface != seat->keyboard_state.focused_surface) return;
...
Monitor *dirtomon(enum wlr_direction dir) {
    ...
    if (!wlr_output_layout_get(output_layout, selmon->wlr_output)) return selmon;
...
void drawbar(Monitor *m) {
    ...
    if (!m->scene_buffer->node.enabled) return;
    if (!(buf = bufmon(m))) return;
...
void focusclient(Client *c, int lift) {
    ...
    if (c && !client_is_unmanaged(c)) {
        ...
        if (!exclusive_focus && !seat->drag) client_set_border_color(c, (float[])COLOR(colors[SchemeSel][ColBorder]));
    }
...
void focusstack(const Arg *arg) {
    ...
    if (!sel || (sel->isfullscreen && !client_has_children(sel)))
        // if sel is null, or if sel is fullscreen and it has no children, return
        return;
    if (arg->i > 0) {
        wl_list_for_each(c, &sel->link, link) {
            if (&c->link == &clients) continue; /* wrap past the sentinel node */
            if (VISIBLEON(c, selmon)) break;    /* found it */
        }
    } else {
...
unset_fullscreen:
    ...
    wl_list_for_each(w, &clients, link) {
        if (w != c && w != p && w->isfullscreen && m == w->mon && (w->tags & c->tags)) setfullscreen(w, 0);
    }
}
    ...
    if (time) {
        ...
        if (active_constraint && cursor_mode != CurResize && cursor_mode != CurMove) {
            ...
            if (c && active_constraint->surface == seat->pointer_state.focused_surface) {
                ...
                if (wlr_region_confine(&active_constraint->region, sx, sy, sx + dx, sy + dy, &sx_confined,
                                       &sy_confined)) {
                    dx = sx_confined - sx;
                    dy = sy_confined - sy;
                }
        ...
        if (sloppyfocus) selmon = xytomon(cursor->x, cursor->y);
    }
...
void moveresizekb(const Arg *arg) {
    Client  *c = focustop(selmon);
    Monitor *m = selmon;

    if (!(m && arg && arg->v && c && c->isfloating)) { return; }
...
void setfloating(Client *c, int floating) {
    ...
    if (!c->mon || !client_surface(c)->mapped || !c->mon->lt[c->mon->sellt]->arrange) return;
...
void setmon(Client *c, Monitor *m, uint32_t newtags) {
    ...
    if (c->swallowing) setmon(c->swallowing, m, newtags);
}
...
void setup(void) {
    ...
    status_event_source =
        wl_event_loop_add_fd(wl_display_get_event_loop(dpy), STDIN_FILENO, WL_EVENT_READABLE, statusin, NULL);
...
int statusin(int fd, unsigned int mask, void *data) {
    ...
    if (mask & WL_EVENT_HANGUP) wl_event_source_remove(status_event_source);

    n = read(fd, status, sizeof(status) - 1);
    if (n < 0 && errno != EWOULDBLOCK) die("read:");
...
Client *termforwin(Client *c) {
    ...
    for (i = 0; i < pids_len; i++) {
        wl_list_for_each(p, &clients, link) {
            if (!p->pid || !p->isterm || p->swallowedby) continue;
            if (pids[i] == p->pid) return p;
        }
    }
...
void tile(Monitor *m) {
    ...
    if (n == smartborders) { draw_borders = 0; }
    ...
    wl_list_for_each(c, &clients, link) {
        ...
        if (i < m->nmaster) {
            ...
            resize(
                c,
                (struct wlr_box){.x = m->w.x + gappx * e, .y = m->w.y + my, .width = mw - 2 * gappx * e, .height = h},
                0, draw_borders);
...
void toggleswallow(const Arg *arg) {
    ...
    if (sel->swallowing) {
        ...
        wl_list_for_each(c, &sel->flink, flink) {
            if (&c->flink == &fstack) continue; /* wrap past the sentinel node */
            if (VISIBLEON(c, selmon)) break;    /* found it */
        }
        swallow(sel, c);
    }
...
void toggleautoswallow(const Arg *arg) { enableautoswallow = !enableautoswallow; }
...
void unmaplayersurfacenotify(struct wl_listener *listener, void *data) {
    ...
    if (l->layer_surface->output && (l->mon = l->layer_surface->output->data)) arrangelayers(l->mon);
    if (l->layer_surface->surface == seat->keyboard_state.focused_surface) focusclient(focustop(selmon), 1);
    motionnotify(0, NULL, 0, 0, 0, 0);
}
...
void updatemons(struct wl_listener *listener, void *data) {
    ...
    wl_list_for_each(m, &mons, link) {
        ...
        if ((c = focustop(m)) && c->isfullscreen) resize(c, m->m, 0, 0);
    ...
    if (stext[0] == '\0') strncpy(stext, "dwl-" VERSION, sizeof(stext));
    wl_list_for_each(m, &mons, link) {
        updatebar(m);
        drawbar(m);
    }
...
void updatebar(Monitor *m) {
    ...
    if (m->b.scale == m->wlr_output->scale && m->drw) return;

    drwl_font_destroy(m->drw->font);
    snprintf(fontattrs, sizeof(fontattrs), "dpi=%.2f", 96. * m->wlr_output->scale);
    if (!(drwl_font_create(m->drw, LENGTH(fonts), fonts, fontattrs))) die("Could not load font");
...
void urgent(struct wl_listener *listener, void *data) {
    ...
    if (client_surface(c)->mapped) client_set_border_color(c, (float[])COLOR(colors[SchemeUrg][ColBorder]));
}
...
void activatex11(struct wl_listener *listener, void *data) {
    ...
    if (!client_is_unmanaged(c)) wlr_xwayland_surface_activate(c->surface.xwayland, 1);
}
